...
// Функция для создания нового узла дерева с проверкой на уникальность
struct Node* CreateTree(struct Node* root, struct Node* r, int data) {
    ...
    // Проверяем, если данные уже есть в дереве
    if (data == r->data) {
        printf("Число %d уже существует в дереве, оно не будет добавлено.\n", data);
        return root; // Возвращаем дерево без изменений
    }

    // Если данные меньше текущего узла, добавляем в левое поддерево
    if (data < r->data)
