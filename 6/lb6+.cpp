...
void merge_vertices(int n, int matrix[MAX_SIZE][MAX_SIZE], int v1, int v2) {
    if (matrix[v1][v1] != 1 || matrix[v2][v2] != 1) {
        printf("Одна из вершин не является корректной петлей. Операция отменена.\n");
        return;
    }
    for (int i = 0; i < n; i++) {
        if (i != v1 && i != v2) {
...
void contract_edge(int* n, int matrix[MAX_SIZE][MAX_SIZE], int v1, int v2) {
    if (matrix[v1][v2] == 0) {
        printf("Ребро между указанными вершинами отсутствует. Операция отменена.\n");
        return;
    }
...
void split_vertex(int* n, int matrix[MAX_SIZE][MAX_SIZE], int v) {
    ...
    matrix[*n][v] = 1; // Создаем связь между исходной и новой вершинами
    matrix[v][*n] = 1;
    matrix[*n][*n] = 0;
    (*n)++;
}
